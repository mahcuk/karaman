...
self.addEventListener('fetch',event=>{
 ...
 if(url.pathname.endsWith('/sw.js')||url.pathname.endsWith('/update.js')||url.pathname.endsWith('/index.html')||url.pathname.endsWith('/')){
  event.respondWith(fetch(event.request,{cache:'no-store'}).then(async r=>{
   if(!r.ok)throw new Error('network');
   if(url.pathname.endsWith('/sw.js')||url.pathname.endsWith('/update.js'))return r;
   let html=await r.text();
   if(!html.includes('./update.js'))html=html.replace('</body>','<script src="./update.js?v=16"></script></body>');
