...
self.addEventListener('fetch',event=>{
 ...
 if(url.pathname.endsWith('/index.html')||url.pathname.endsWith('/')){
  event.respondWith(fetch(event.request,{cache:'no-store'}).then(async r=>{if(!r.ok)throw new Error('network');const text=await r.text();const enhanced=text.replace('</body>','<script src="./override.js?v=11"></script><script src="./route-fix.js?v=11"></script><script src="./route-final.js?v=11"></script><script src="./final-fix.js?v=11"></script></body>');const response=new Response(enhanced,{status:r.status,statusText:r.statusText,headers:r.headers});caches.open(CACHE).then(c=>c.put(event.request,response.clone()));return response}).catch(()=>caches.match(event.request).then(r=>r||caches.match('./index.html'))));return;
 }
